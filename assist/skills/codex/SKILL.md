...
description: >
  ...
  or when you want a fast second perspective. Lighter and faster than a full review board.
license: MIT
compatibility: Claude Code, OpenCode, Pi
allowed-tools: Bash(bash:assist/skills/codex/*) Write(*/codex-*)
user-invocable: true
...
|------|---------|----------|
| **ask** (default) | `bash assist/skills/codex/scripts/run-codex.sh ask` | Ask a specific question with context |
| **review** | `bash assist/skills/codex/scripts/run-codex.sh review [flags]` | Review code changes via `codex exec review` |
...
2. **Resolve the temp directory** (run once, reuse for all files in this session):
   ```bash
   bash assist/skills/codex/scripts/resolve-tmp.sh
   ```
...
```bash
bash assist/skills/codex/scripts/run-codex.sh ask <TMP>/codex-<ID>-question.md
```
...
- Override by passing a number as the last argument: `bash assist/skills/codex/scripts/run-codex.sh ask 120`
