...
|------|---------|----------|
| **ask** (default) | `bash assist/codex/scripts/run-codex.sh ask` | Ask a specific question with context |
| **review** | `bash assist/codex/scripts/run-codex.sh review [flags]` | Review code changes via `codex exec review` |
...
2. **Resolve the temp directory** (run once, reuse for all files in this session):
   ```bash
   bash assist/codex/scripts/resolve-tmp.sh
   ```
...
```bash
bash assist/codex/scripts/run-codex.sh ask <TMP>/codex-${CLAUDE_SESSION_ID}-question.md
```
...
```bash
bash assist/codex/scripts/run-codex.sh review --uncommitted
```
...
- Override by passing a number as the last argument: `bash assist/codex/scripts/run-codex.sh ask 120`
