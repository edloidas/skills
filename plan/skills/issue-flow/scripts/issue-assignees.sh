#!/bin/bash
# issue-assignees.sh
# Find users actually assigned to recent issues.
# Output: <user>\t<count> per row, up to 3 rows, sorted by count desc.
# Excludes the authenticated user and common bot accounts.
#
# Usage: issue-assignees.sh [<owner>/<repo>] [<limit>]
#   <owner>/<repo>  Defaults to current repo (gh repo view).
#   <limit>         Issue scan window (default 100).
#
# Notes:
#   - Uses --state all to capture both open and recently closed issues.
