...
description: >
  ...
  review boards, debugging, or research that need to be fixed in the current project.
...
**Without `AskUserQuestion`** (any host other than Claude Code): present the same scope
decision inline as a numbered list of 2–5 options, recommended first, and wait for the user
to reply with an option number. Do not silently fall back to auto mode — auto mode is an
explicit `$ARGUMENTS` opt-in, not a default for hosts that lack a question tool.
