...
Forward-looking guidance for a future calibration skill that will consume `code-to-spec` output and regression-test prompt changes. No calibration harness ships with this skill in v0.1 — the user has elected to build that as a separate skill later.

## Purpose of calibration

Calibration is a regression gate. A frozen reference bundle plus its expected audit summary defines "acceptable spec quality." Any change to the code-to-spec prompts must keep existing references passing.
...
2. Run `code-to-spec` on the bundle.
...
No machine-readable schema is emitted by `code-to-spec` today — the parser would grep for `## Critical (N)`, `## Warning (N)`, `## Note (N)` headers and count by section. If a future version adds JSON frontmatter or a machine-readable summary file, the parser can switch to it.
